...
const DataNews: React.FC = () => {
  ...
  const renderLineChart = () => (
    <div className="chart-container">
      ...
      <div className="line-chart">
        ...
        <div className="chart-area">
          <div className="x-axis">
            {yearLabels.map(year => (
              <div key={year} className="x-label">{year}</div>
            ))}
          </div>
          {incomeData.map(cityData => (
            <div key={cityData.city} className="line-group">
              <svg viewBox="0 0 400 200" preserveAspectRatio="none" className="line-svg">
                <defs>
                  <linearGradient id={`gradient-${cityData.city}`} x1="0%" y1="0%" x2="0%" y2="100%">
                ...
                />
                <path
                  d={`M 0,200 L ${cityData.data.map((val, i) => 
                    `${(i / 4) * 400},${200 - (val / maxIncome) * 200}`
                  ).join(' L ')} L 400,200 Z`}
                  fill={`url(#gradient-${cityData.city})`}
                />
                {cityData.data.map((val, i) => (
                  <circle
                    key={i}
                    cx={(i / 4) * 400}
                    cy={200 - (val / maxIncome) * 200}
                    r="5"
      ...
      <div className="legend">
        {incomeData.map(cityData => (
          <div key={cityData.city} className="legend-item">
            <span className="legend-color" style={{ backgroundColor: colors[cityData.city as keyof typeof colors] }}></span>
  ...
  const renderBarChart = () => (
    <div className="chart-container">
      ...
      <div className="bar-chart-horizontal">
        {heatData.map((item) => (
          <div key={item.city} className="bar-row">
            ...
            <span className="bar-tag">{item.label}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="bar-chart-grouped">
        <div className="grouped-bars">
          <div className="bar-group">
            <div className="bar-title">短期爆火型</div>
            <div className="bars">
              <div className="bar-item">
                <div className="bar" style={{ height: `${(720 / 2490) * 100}%`, backgroundColor: colors['淄博'] }}></div>
                <span>淄博</span>
                <span className="bar-amount">720亿</span>
              </div>
              <div className="bar-item">
                <div className="bar" style={{ height: `${(980 / 2490) * 100}%`, backgroundColor: colors['哈尔滨'] }}></div>
                <span>哈尔滨</span>
                <span className="bar-amount">980亿</span>
              </div>
            </div>
          </div>
          <div className="bar-group">
            <div className="bar-title">长效文旅型</div>
            <div className="bars">
              <div className="bar-item">
                <div className="bar" style={{ height: `${(2130 / 2490) * 100}%`, backgroundColor: colors['西安'] }}></div>
                <span>西安</span>
                <span className="bar-amount">2130亿</span>
              </div>
              <div className="bar-item">
                <div className="bar" style={{ height: `${(2490 / 2490) * 100}%`, backgroundColor: colors['长沙'] }}></div>
                <span>长沙</span>
                <span className="bar-amount">2490亿</span>
              </div>
            </div>
          </div>
        </div>
  ...
  const renderHeatChart = () => {
    ...
    return (
      <div className="chart-container">
        <h3>🔥 网红城市季节性热度波动分析</h3>
        <div className="heat-chart-new">
          <div className="heat-header">
            <div className="city-label-header"></div>
            <div className="season-header">
              {['春', '夏', '秋', '冬'].map(season => (
                <div key={season} className="season-header-cell">{season}</div>
              ))}
            </div>
          </div>
          {incomeData.map(city => {
            const isPulse = ['淄博', '哈尔滨', '洛阳'].includes(city.city);
            const cityData = seasonalData[city.city as keyof typeof seasonalData];
            
            return (
              <div key={city.city} className="heat-row">
                <div className={`city-label ${isPulse ? 'pulse-city' : 'stable-city'}`}>
                  <span className="city-name">{city.city}</span>
                  <span className={`city-type ${isPulse ? 'pulse' : 'stable'}`}>
                    {isPulse ? '脉冲式' : '长效型'}
                  </span>
                </div>
                <div className="heat-cells">
                  {cityData.map((value, index) => {
                    ...
                    const season = ['春', '夏', '秋', '冬'][index];
                    
                    return (
                      <div 
                        key={index}
                        className={`heat-cell-new ${isHighValue ? 'high' : ''} ${isLowValue ? 'low' : ''}`}
                        style={{ backgroundColor: `rgba(231, 76, 60, ${value / 100})` }}
                        title={`${city.city} ${season}: ${value}%`}
                      >
          ...
          <div className="heat-legend-new">
            <div className="legend-item">
              <span className="legend-color" style={{ backgroundColor: 'rgba(231, 76, 60, 0.1)' }}></span>
              <span>低</span>
            </div>
            <div className="legend-gradient-new"></div>
            <div className="legend-item">
              <span className="legend-color" style={{ backgroundColor: 'rgba(231, 76, 60, 1)' }}></span>
              <span>高</span>
            </div>
          </div>
          <div className="heat-analysis">
            <div className="analysis-item">
              ...
              <span><strong>脉冲式热度</strong>：淄博（夏季烧烤90%）、哈尔滨（冬季冰雪95%）、洛阳（春季牡丹95%）热度高度集中于单一季节</span>
            </div>
            <div className="analysis-item">
              <span className="marker stable"></span>
              <span><strong>长效型热度</strong>：长沙、西安全年热度分布均匀（70%-85%），依托成熟文旅IP实现稳定流量留存</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="data-news">
      <div className="news-header">
        <h1>数据洞察：近5年网红文旅城市热度分化</h1>
        <p className="subtitle">谁在靠流量赚钱，谁只是昙花一现</p>
        <p className="date">2025年5月 | 数据新闻专题</p>
      </div>
      ...
      <div className="news-section">
        ...
        <div className="data-table">
          ...
          <p className="table-caption">单位：亿元</p>
        </div>

        <div className="highlight-box">
          <h3>百度指数·网红城市热度峰值对比</h3>
          <div className="ranking">
            <div className="rank-item top">
              <span className="rank">1</span>
              <span className="city">哈尔滨（冰雪季）</span>
            </div>
            <div className="rank-item">
              <span className="rank">2</span>
              <span className="city">淄博（烧烤季）</span>
            </div>
            <div className="rank-item">
              <span className="rank">3</span>
              <span className="city">长沙</span>
            </div>
            <div className="rank-item">
              <span className="rank">4</span>
              <span className="city">西安</span>
            </div>
            <div className="rank-item">
              <span className="rank">5</span>
              <span className="city">洛阳</span>
            </div>
          </div>
        </div>

        <div className="peak-chart-intro">
          <h3>📊 网红城市网络热度峰值对比</h3>
          <div className="bar-chart-horizontal">
            {heatData.map((item) => (
              <div key={item.city} className="bar-row">
          ...
          <div className="peak-insight">
            ...
            <p><strong>热度峰值 ≠ 旅游收入</strong>：哈尔滨热度最高(95)，但2024年收入仅980亿；长沙热度第三(75)，收入却最高达2490亿。</p>
          </div>
        </div>

        <div className="insight-section">
          <div className="insight-card stable">
            <h4>🏛️ 长效文旅型（稳定留存）</h4>
      ...
      <div className="news-section">
        ...
        <div className="chart-tabs">
          {[
            { id: 'line', label: '收入趋势' },
            { id: 'bar', label: '热度对比' },
            { id: 'comparison', label: '类型对比' },
            { id: 'heat', label: '季节波动' },
          ].map(tab => (
            <button
              key={tab.id}
              className={`tab-btn ${activeChart === tab.id ? 'active' : ''}`}
        ...
        <div className="chart-content">
          ...
          {activeChart === 'heat' && renderHeatChart()}
        </div>
      </div>
